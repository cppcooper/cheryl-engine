...
namespace CE::Assets {
    namespace {
        ...
        void require_object(const json& value, const fs::path& source,
        ...
        void require_array(const json& value, const fs::path& source,
                           const std::string_view location) {
            if (!value.is_array()) {
        ...
        void allow_only(const json& object, const fs::path& source,
                        const std::string_view location,
        ...
        const json& required(const json& object, const std::string_view key,
        ...
        std::string read_string(const json& value, const fs::path& source,
                                const std::string_view location, const bool allow_empty = false) {
            if (!value.is_string()) {
        ...
        std::string description(const json& object, const fs::path& source,
                                const std::string_view location) {
            if (!object.contains("description")) {
                return {};
            }
            return read_string(object.at("description"), source,
                               std::string(location) + ".description");
        }

        std::uint64_t read_unsigned(const json& value, const fs::path& source,
                                    const std::string_view location,
        ...
        std::size_t read_size(const json& value, const fs::path& source,
                              const std::string_view location, const std::size_t minimum = 0) {
            return static_cast<std::size_t>(read_unsigned(
                value, source, location, minimum, std::numeric_limits<std::size_t>::max()));
        }

        std::uint32_t read_u32(const json& value, const fs::path& source,
                               const std::string_view location, const std::uint32_t minimum = 0) {
            return static_cast<std::uint32_t>(read_unsigned(
                value, source, location, minimum, std::numeric_limits<std::uint32_t>::max()));
        }

        double read_positive_number(const json& value, const fs::path& source,
                                    const std::string_view location) {
            if (!value.is_number()) {
        ...
        std::chrono::milliseconds read_duration(const json& value, const fs::path& source,
                                                const std::string_view location) {
            const auto count = read_unsigned(value, source, location, 1,
        ...
        bool is_identifier(const std::string_view value, const bool allow_dot) {
            ...
            return std::ranges::all_of(value, [allow_dot](const char ch) {
                return (ch >= 'a' && ch <= 'z') || (ch >= '0' && ch <= '9')
                    || ch == '_' || ch == '-' || (allow_dot && ch == '.');
            });
        }

        std::string read_identifier(const json& value, const fs::path& source,
                                    const std::string_view location, const bool allow_dot = false) {
        ...
        void validate_map_key(const std::string& key, const fs::path& source,
                              const std::string_view location) {
        ...
        math::Pivot parse_pivot(const json& value, const fs::path& source,
                                const std::string_view location) {
        ...
        fs::path parse_texture_path(const json& value, const fs::path& source,
                                    const std::string_view location) {
        ...
        GridDefinition parse_grid(const json& value, const fs::path& source,
                                  const std::string_view location) {
            allow_only(value, source, location,
                       {"origin", "frame", "spacing", "rows", "columns", "cell_order"});
        ...
        CellIndex parse_cell(const json& value, const GridDefinition& grid,
                             const fs::path& source, const std::string_view location) {
            require_object(value, source, location);
            if (value.contains("index")) {
                allow_only(value, source, location, {"index"});
                const auto index = read_size(value.at("index"), source,
            ...
            const auto row = read_size(required(value, "row", source, location), source,
                                       std::string(location) + ".row");
            const auto column = read_size(required(value, "column", source, location), source,
        ...
        std::unordered_map<std::string, ViewDefinition> parse_views(
            const json& value, const GridDefinition& grid, const fs::path& source,
            const std::string_view location) {
            require_object(value, source, location);
            if (value.empty()) {
            ...
            for (const auto& [name, view] : value.items()) {
                ...
                allow_only(view, source, view_location,
                           {"description", "row", "column", "rows", "columns"});
                ViewDefinition definition {
                    .description = description(view, source, view_location),
                    .row = read_size(required(view, "row", source, view_location), source,
                                     view_location + ".row"),
                    .column = read_size(required(view, "column", source, view_location), source,
                                        view_location + ".column"),
                    .rows = read_size(required(view, "rows", source, view_location), source,
                                      view_location + ".rows", 1),
        ...
        std::unordered_map<std::string, CellIndex> parse_orientations(
            ...
            for (const auto& [name, cell] : value.items()) {
                validate_map_key(name, source, location);
                result.emplace(name, parse_cell(cell, grid, source,
        ...
        std::vector<TimedFrameDefinition> parse_frames(
            ...
            for (std::size_t index = 0; index < value.size(); ++index) {
                ...
                result.push_back({
                    parse_cell(required(frame, "cell", source, frame_location), grid,
                               source, frame_location + ".cell"),
                    read_duration(required(frame, "duration_ms", source, frame_location),
                                  source, frame_location + ".duration_ms")
                });
            }
            return result;
        }

        std::unordered_map<std::string, AnimationProfileDefinition> parse_profiles(
            const json& value, const fs::path& source, const std::string_view location) {
            require_object(value, source, location);
            if (value.empty()) {
            ...
            for (const auto& [profile_name, profile] : value.items()) {
                ...
                for (const auto& [name, offset] : facings.items()) {
                    ...
                    definition.facings.emplace(
                        name, read_size(offset, source, profile_location + ".facings." + name));
                }
                ...
                for (const auto& [name, clip] : clips.items()) {
                    ...
                    clip_definition.row_offset = read_size(
                        required(clip, "row_offset", source, clip_location), source,
                    ...
                    clip_definition.frame_duration = read_duration(
                        required(clip, "frame_duration_ms", source, clip_location), source,
                    ...
                    for (const auto column : clip.columns) {
                        if (column >= sprite.grid.columns) {
                            fail(source, location,
                                 "animation profile selects a column outside the sprite grid");
                        }
                        animation.frames.push_back({sprite.grid.cell_index(row, column),
                                                    clip.frame_duration});
                    }
        ...
        void append_explicit_animations(SpriteDefinition& sprite, const json& value,
                                        const fs::path& source, const std::string_view location) {
            require_object(value, source, location);
            if (value.empty()) {
        ...
        Direction parse_direction(const json& value, const fs::path& source,
                                  const std::string_view location) {
            const auto name = read_string(value, source, location);
            const auto iter = std::ranges::find_if(directions, [&name](const auto& item) {
        ...
        WangAutotileDefinition parse_wang_autotile(
            const json& value, const GridDefinition& grid, const WangType type,
            const fs::path& source, const std::string_view location) {
            allow_only(value, source, location,
                       {"description", "type", "slot_order", "terrains", "tiles"});
            WangAutotileDefinition result {
            ...
            for (std::size_t index = 0; index < result.slot_order.size(); ++index) {
                result.slot_order[index] = parse_direction(
                    slots[index], source,
                    std::string(location) + ".slot_order[" + std::to_string(index) + ']');
            ...
            for (std::size_t index = 0; index < terrains.size(); ++index) {
                ...
                allow_only(terrain, source, terrain_location,
                           {"id", "name", "color", "probability", "representative"});
                TerrainDefinition definition {
                    .id = read_u32(required(terrain, "id", source, terrain_location), source,
                                   terrain_location + ".id", 1),
                    .name = read_string(required(terrain, "name", source, terrain_location), source,
                                        terrain_location + ".name"),
                    .color = std::nullopt,
                    .probability = 1.0,
                ...
                if (terrain.contains("color")) {
                    const auto color = read_string(terrain.at("color"), source,
                                                   terrain_location + ".color");
                ...
                if (terrain.contains("probability")) {
                    definition.probability = read_positive_number(
                        terrain.at("probability"), source, terrain_location + ".probability");
                }
                if (terrain.contains("representative")) {
                    definition.representative = parse_cell(
                        terrain.at("representative"), grid, source,
                        terrain_location + ".representative");
                }
            ...
            for (std::size_t index = 0; index < tiles.size(); ++index) {
                ...
                WangTileDefinition definition {
                    .cell = parse_cell(required(tile, "cell", source, tile_location), grid,
                ...
                for (std::size_t slot = 0; slot < definition.wang.size(); ++slot) {
                    definition.wang[slot] = read_u32(signature[slot], source,
                                                     tile_location + ".wang[" + std::to_string(slot) + ']');
                    ...
                    if (definition.wang[slot] != 0
                        && ((type == WangType::Corner && is_edge_slot)
                            || (type == WangType::Edge && !is_edge_slot))) {
                        fail(source, tile_location + ".wang",
                             "Wang signature uses a slot incompatible with its type");
                    }
                }
                if (tile.contains("weight")) {
                    definition.weight = read_positive_number(tile.at("weight"), source,
                                                             tile_location + ".weight");
                }
        ...
        BitmaskAutotileDefinition parse_bitmask_autotile(
            const json& value, const GridDefinition& grid, const BitmaskType type,
            const fs::path& source, const std::string_view location) {
            ...
            for (std::size_t index = 0; index < order.size(); ++index) {
                const auto direction = parse_direction(
                    order[index], source,
                    std::string(location) + ".bit_order[" + std::to_string(index) + ']');
            ...
            for (const auto& [mask_text, cell] : cases.items()) {
                std::uint32_t mask{};
                const auto [end, error] = std::from_chars(mask_text.data(),
                                                          mask_text.data() + mask_text.size(), mask);
                if (error != std::errc{} || end != mask_text.data() + mask_text.size()
                    || (mask_text.size() > 1 && mask_text.front() == '0') || mask > maximum_mask) {
                    fail(source, std::string(location) + ".cases." + mask_text,
                         "invalid or out-of-range bitmask");
                }
                result.cases.emplace(mask, parse_cell(
                    cell, grid, source, std::string(location) + ".cases." + mask_text));
            }
            return result;
        }
        ...
        std::unordered_map<std::string, AutotileDefinition> parse_autotiles(
            ...
            for (const auto& [name, autotile] : value.items()) {
                ...
                const auto type = read_string(required(autotile, "type", source, autotile_location),
                                              source, autotile_location + ".type");
                if (type == "wang-corner") {
                    result.emplace(name, parse_wang_autotile(
                        autotile, grid, WangType::Corner, source, autotile_location));
                } else if (type == "wang-edge") {
                    result.emplace(name, parse_wang_autotile(
                        autotile, grid, WangType::Edge, source, autotile_location));
                } else if (type == "four-neighbor") {
                    result.emplace(name, parse_bitmask_autotile(
                        autotile, grid, BitmaskType::FourNeighbor, source, autotile_location));
                } else if (type == "eight-neighbor") {
                    result.emplace(name, parse_bitmask_autotile(
                        autotile, grid, BitmaskType::EightNeighbor, source, autotile_location));
                } else {
        ...
        public:
            ...
            AssetManifest parse(const json& root) const {
                allow_only(root, source_, "$",
                           {"$schema", "version", "namespace", "texture", "defaults",
                            "animation_profiles", "sprites", "tilesets"});
                AssetManifest manifest;
                manifest.source = source_;
                manifest.schema = read_string(required(root, "$schema", source_, "$"),
                                              source_, "$.$schema");
                if (manifest.schema != "./schemas/asset-manifest-1.0.schema.json") {
                    fail(source_, "$.$schema", "unsupported asset manifest schema");
                }
                manifest.version = read_string(required(root, "version", source_, "$"),
                                               source_, "$.version");
                if (manifest.version != "1.0") {
                    fail(source_, "$.version", "unsupported asset manifest version '" + manifest.version + "'");
                }
                manifest.name_space = read_identifier(
                    required(root, "namespace", source_, "$"), source_, "$.namespace", true);
                ...
                manifest.default_sprite_pivot = parse_pivot(
                    required(sprite_defaults, "pivot", source_, "$.defaults.sprite"),
                    source_, "$.defaults.sprite.pivot");
                ...
                manifest.default_tileset_pivot = parse_pivot(
                    required(tileset_defaults, "pivot", source_, "$.defaults.tileset"),
                    source_, "$.defaults.tileset.pivot");
                ...
                if (root.contains("animation_profiles")) {
                    manifest.animation_profiles = parse_profiles(
                        root.at("animation_profiles"), source_, "$.animation_profiles");
                }
        ...
        private:
            ...
            fs::path entry_texture(const json& entry, const AssetManifest& manifest,
                ...
                if (entry.contains("texture")) {
                    return parse_texture_path(entry.at("texture"), source_,
                                              std::string(location) + ".texture");
                }
                if (manifest.texture) {
            ...
            void parse_sprites(const json& value, AssetManifest& manifest) const {
                ...
                for (const auto& [name, sprite] : value.items()) {
                    ...
                    allow_only(sprite, source_, location,
                               {"description", "texture", "grid", "pivot", "views",
                                "orientations", "animation_profile", "animations"});
                    SpriteDefinition definition {
                        ...
                        .grid = parse_grid(required(sprite, "grid", source_, location),
                                           source_, location + ".grid"),
                    ...
                    if (sprite.contains("views")) {
                        definition.views = parse_views(sprite.at("views"), definition.grid,
                                                       source_, location + ".views");
                    }
                    if (sprite.contains("orientations")) {
                        definition.orientations = parse_orientations(
                            sprite.at("orientations"), definition.grid, source_,
                            location + ".orientations");
                    }
                    if (sprite.contains("animation_profile")) {
                        const auto profile_name = read_identifier(
                            sprite.at("animation_profile"), source_, location + ".animation_profile");
            ...
            void parse_tilesets(const json& value, AssetManifest& manifest) const {
                ...
                for (const auto& [name, tileset] : value.items()) {
                    ...
                    allow_only(tileset, source_, location,
                               {"description", "texture", "grid", "pivot", "views",
                                "orientations", "animations", "autotiles"});
                    TilesetDefinition definition {
                        ...
                        .grid = parse_grid(required(tileset, "grid", source_, location),
                                           source_, location + ".grid"),
                    ...
                    if (tileset.contains("views")) {
                        definition.views = parse_views(tileset.at("views"), definition.grid,
                                                       source_, location + ".views");
                    }
                    if (tileset.contains("orientations")) {
                        definition.orientations = parse_orientations(
                            tileset.at("orientations"), definition.grid, source_,
                            location + ".orientations");
                    }
                    if (tileset.contains("animations")) {
                        ...
                        for (const auto& [animation_name, animation] : animations.items()) {
                            ...
                            TileAnimationDefinition animation_definition {
                                ...
                                .target = parse_cell(
                                    required(animation, "target", source_, animation_location),
                                    definition.grid, source_, animation_location + ".target"),
                                .frames = parse_frames(
                                    required(animation, "frames", source_, animation_location),
                            ...
                            definition.animations.emplace(
                                animation_name, std::move(animation_definition));
                        }
                    }
                    if (tileset.contains("autotiles")) {
                        definition.autotiles = parse_autotiles(
                            tileset.at("autotiles"), definition.grid, source_,
                            location + ".autotiles");
                    }
    ...
    AssetManifest ManifestLoader::parse(std::istream& input, const std::filesystem::path& source) {
        try {
            ...
            throw std::runtime_error(
                "Unable to parse asset manifest '" + source.string() + "': " + error.what());
        }
    }
}
