...
namespace CE::Assets {
    GridGeometry make_grid_geometry(const GridDefinition& grid, const math::Pivot pivot,
        ...
        const auto vertex_count = static_cast<std::uint32_t>(
            cell_count * VAONumbers::vertices_per_quad);
        ...
        auto vertices = std::shared_ptr<Vertex2D>(
            static_cast<Vertex2D*>(block.head.get()), [block](Vertex2D*) {
                Mem::ExactMMgr::get().return_chunk(block);
            });
        ...
        for (CellIndex cell = 0; cell < cell_count; ++cell) {
            ...
            math::Anchor::MakePivot(
                pivot, vertices.get() + cell * VAONumbers::vertices_per_quad,
