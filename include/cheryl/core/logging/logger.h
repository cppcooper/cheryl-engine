...
namespace CE {
    ...
    public:
        static void set_pattern(const char* fmt) {
            Singleton_CTS<Log<name>>::get().set_pattern(fmt);
        }

        [[nodiscard]] static std::filesystem::path get_file_path() {
            return Singleton_CTS<Log<name>>::get().get_file_path();
        }

        static void flush() {
            Singleton_CTS<Log<name>>::get().flush();
        }
        ...
        static void reopen() {
            Singleton_CTS<Log<name>>::get().reopen();
        }

        static void make_default() {
            Singleton_CTS<Log<name>>::get().make_default();
        }

        static void set_level_logger(spdlog::level level) {
            Singleton_CTS<Log<name>>::get().set_level_logger(level);
        }

        static void set_level_filesink(spdlog::level level) {
            Singleton_CTS<Log<name>>::get().set_level_filesink(level);
        }

        static void set_level_stdsink(spdlog::level level) {
            Singleton_CTS<Log<name>>::get().set_level_stdsink(level);
        }
